...
def topTen():
    if(request.method=="GET"):
        ...
        if(industry == None):
            ...
            results.sort(key=sort_func)
            results.reverse()
            return json.dumps(results[0:10], default=json_util.default)
...
def get_document(document):
    try:
        ...
        db = connection["market"]
        collection = db["stocks"]
        x = collection.find_one({ "Company": document })
        print(x)
        return x
...
def get_by_sector(sector):
    cursor = collection.find( { "Sector":  sector } )
    dic = {}
    for record in cursor:
        dic = record
        dic.pop('_id', None)
...
#curl -H "Content-Type: application/json" -X POST -d '{"Ticker": "EEM", "Sector": "Financial", "Change from Open": 0.0152, "200-Day Simple Moving Average": 0.0087, "52-Week High": -0.0735, "Change": 0.0159, "Country": "USA", "50-Day Low": 0.065, "Price": 41.45, "50-Day High": -0.056, "Dividend Yield": 0.0181, "Industry": "Exchange Traded Fund", "52-Week Low": 0.1615, "Average True Range": 0.54, "Company": "iShares MSCI Emerging Markets Index", "Gap": 0.0007, "Relative Volume": 1.07, "Volume": "", "20-Day Simple Moving Average": -0.0212, "Average Volume": 64990.81, "50-Day Simple Moving Average": -0.0126}' http://localhost:8080/createStock

#curl http://localhost:8080/getStock?business_name="ACME%TEST%INC."
